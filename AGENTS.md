...
1.  Define the manager class in `src/core/behavior/` or `src/core/resources/`.
2.  Initialize it in the `AgentCore` or `Agent` constructor.
3.  If it needs to be updated every frame, call its `update(deltaTime)` method inside `Agent.startLoop`.
...
-   The Balloon uses **Shadow DOM**. Styles are defined in `src/ui/Balloon.ts` using a `<style>` tag.
-   Visual changes to the bubble shape should be made in the SVG path generation logic in `Balloon._updatePath`.

### Recipe: Modifying Behavioral Logic (XState)
-   The high-level logic (Idling, Busy, Hiding) is defined in `src/core/behavior/AgentMachine.ts`.
-   To add a new state, update the `states` object in the machine definition.
-   To handle timing deterministically, use the `TICK` event which provides `deltaTime` from the main loop.
...
-   **JSDoc**: The codebase is heavily documented. Use the `read_file` tool on `src/core/base/types.ts` to understand the primary data structures.
